...
function NewReleasesPage({ onPlay, selectedTrack, isPlaying, setIsPlaying }) {
    ...
    const fetchNewReleases = async () => {
        setLoading(true);
        try {
            const tracksResponse = await axios.get('http://localhost:3000/api/tracks/recent-user');
            const tracksData = tracksResponse.data || [];
            setTracks(tracksData);

            const albumsResponse = await axios.get('http://localhost:3000/api/albums/recent-user');
            const albumsData = albumsResponse.data || [];
            setAlbums(albumsData);

            const artistsResponse = await axios.get('http://localhost:3000/api/artists/recent-user');
