...
// Компонент для отображения одной секции с карточками
...
function NewReleasesPage({ onPlay, selectedTrack, isPlaying, setIsPlaying }) {
    ...
    const fetchNewReleases = async () => {
        setLoading(true);
        try {
            const tracksResponse = await axios.get(`${API_URL}/api/tracks/recent-user`);
            const tracksData = tracksResponse.data || [];
            setTracks(tracksData);

            const albumsResponse = await axios.get(`${API_URL}/api/albums/recent-user`);
            const albumsData = albumsResponse.data || [];
            setAlbums(albumsData);

            const artistsResponse = await axios.get(`${API_URL}/api/artists/recent-user`);
